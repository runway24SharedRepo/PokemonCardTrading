...
def pikachu_fixture() -> dict:
    return {
        ...
        "tcgplayer": {"prices": {"normal": {"market": 12.38}}},
        "cardmarket": {
...
def test_pikachu_uses_30_day_average_not_stale_nonwindowed_average() -> None:
    row = next(
        value
        for value in build_price_variants(pikachu_fixture(), fx(), {})
        if value.variant == "Normal"
    )
    assert row.card_id == "base1-58"
    assert row.source_field == "cardmarket.prices.avg30"
    assert row.original_price == 6.21
    assert row.price_gbp == 5.32
    assert row.match_status == "EXACT CARDMARKET 30-DAY AVERAGE"
    assert row.original_price != 23.99


def test_palkia_holo_uses_30_day_average() -> None:
    ...
    assert holo.source_field == "cardmarket.prices.avg30"
    assert holo.original_price == 14.10
    assert holo.original_currency == "EUR"
    assert holo.exchange_rate_to_gbp == 0.857050
    assert holo.price_gbp == 12.08
    assert holo.match_status == "EXACT CARDMARKET 30-DAY AVERAGE"

    assert reverse.source_field == "cardmarket.prices.reverseHoloAvg30"
    assert reverse.original_price == 10.50
    assert reverse.price_gbp == 9.00


def test_tcgplayer_market_and_trend_are_never_used() -> None:
    card = palkia_fixture()
    card["cardmarket"]["prices"].pop("avg30")
    row = next(
        value
...
def test_reverse_nonwindowed_average_is_not_substituted() -> None:
    card = palkia_fixture()
    card["cardmarket"]["prices"].pop("reverseHoloAvg30")
    row = next(
        value
        for value in build_price_variants(card, fx(), {})
        if value.variant == "Reverse Holofoil"
    )
    assert row.price_gbp is None
    assert row.source_field == "cardmarket.prices.reverseHoloAvg30"
    assert card["cardmarket"]["prices"]["reverseHoloSell"] == 12.00
...
def test_first_edition_is_not_given_unlimited_average() -> None:
    ...
    assert unlimited.price_gbp == 1.80
    assert unlimited.source_field == "cardmarket.prices.avg30"
...
def test_configuration_selects_average_selling_price() -> None:
    ...
    assert config["pricing_policy"] == "cardmarket_30_day_average_selling_price_only"
...
def test_old_metric_is_excluded_from_new_baseline() -> None:
    with tempfile.TemporaryDirectory() as folder:
        ...
        assert database.current_price_map("cardmarket_30_day_average_selling_price_only") == {}
        database.close()
...
def test_database_commit_stores_new_metric() -> None:
    with tempfile.TemporaryDirectory() as folder:
        ...
        metric = "cardmarket_30_day_average_selling_price_only"
...
if __name__ == "__main__":
    ...
    pikachu = next(
        row
        for row in build_price_variants(pikachu_fixture(), fx(), {})
    ...
    print()
    print("PIKACHU BASE 58 ACCEPTANCE RESULT")
    print(f"Card ID: {pikachu.card_id}")
    print(f"Variant: {pikachu.finish} / {pikachu.edition}")
    print(f"Selected JSON field: {pikachu.source_field}")
    print(f"30-Day Average Sell EUR: {pikachu.original_price:.2f}")
    print(f"EUR to GBP: {pikachu.exchange_rate_to_gbp:.6f}")
    print(f"Converted GBP: {pikachu.price_gbp:.2f}")
    print(f"Status: {pikachu.match_status}")
    print()
    ...
    print(f"30-Day Average Sell EUR: {palkia.original_price:.2f}")
    ...
    print(f"PASS: {len(tests)} Phase 5.6.7 tests")
