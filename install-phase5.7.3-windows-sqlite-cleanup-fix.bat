...
echo ======================================================
echo Phase 5.7.3 - Windows SQLite Cleanup Fix
echo ======================================================
echo.
...
set "TESTLOG=logs\phase5.7.3-install-test-%STAMP%.log"
...
if not exist "payload\on_demand_pricing.py" (
  echo ERROR: The Phase 5.7.3 payload is missing or incomplete.
  goto :failed
)

if not exist "tests\test_on_demand_pricing.py" (
  echo ERROR: The Phase 5.7.3 regression test is missing.
  goto :failed
)
...
set "BACKUP=backups\phase5.7.3-source-before-%STAMP%"
