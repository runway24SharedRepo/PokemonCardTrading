...
echo ==============================================================
echo Phase 5.8.1 - Custom Live Result-Sheet Hotfix
echo ==============================================================
echo.
...
set "TESTLOG=logs\phase5.8.1-install-test-%STAMP%.log"
...
if not exist "payload\live_scan_custom.py" (
  echo ERROR: The Phase 5.8.1 payload is incomplete.
  goto :failed
)
...
echo.
if not "%TEST_RC%"=="0" (
  echo ERROR: Phase 5.8.1 tests failed with exit code %TEST_RC%.
  goto :failed
)

set "BACKUP=backups\phase5.8.1-source-before-%STAMP%"
