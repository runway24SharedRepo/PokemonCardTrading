...
echo ==============================================================
echo Phase 5.8.2 - Excel COM Worksheet-Copy Hotfix
echo ==============================================================
echo.
...
set "TESTLOG=logs\phase5.8.2-install-test-%STAMP%.log"
...
if not exist "payload\live_scan_custom.py" (
  echo ERROR: The Phase 5.8.2 payload is incomplete.
  goto :failed
)
...
echo.
if not "%TEST_RC%"=="0" (
  echo ERROR: Phase 5.8.2 tests failed with exit code %TEST_RC%.
  goto :failed
)

set "BACKUP=backups\phase5.8.2-source-before-%STAMP%"
