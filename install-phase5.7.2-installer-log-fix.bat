...
echo ======================================================
echo Phase 5.7.2 - Pricing Cache Installer and Log Fix
echo ======================================================
echo.
...
set "TESTLOG=logs\phase5.7.2-install-test-%STAMP%.log"
...
if exist ".venv\Scripts\python.exe" (
  ...
  py -3 -m py_compile payload\on_demand_pricing.py payload\live_opportunity_radar.py payload\random_range_sniper.py payload\seller_radar.py payload\live_radar\excel_adapter.py payload\random_sniper\excel_adapter.py payload\random_sniper\core.py payload\long_term_excel.py payload\seller_radar_excel.py > "%TESTLOG%" 2>&1
  set "TEST_RC=!ERRORLEVEL!"
  if "!TEST_RC!"=="0" (
...
set "BACKUP=backups\phase5.7.2-source-before-%STAMP%"
...
echo Temporary API failures are deferred instead of blocking the full scan.
goto :success
