...
echo ======================================================
echo Phase 5.7.6 - Scanner Watchlist Write Removal
echo ======================================================
echo.
...
set "TESTLOG=logs\phase5.7.6-install-test-%STAMP%.log"
...
if not exist "payload\on_demand_pricing.py" (
  echo ERROR: The Phase 5.7.6 payload is missing or incomplete.
  goto :failed
)

if not exist "tests\test_on_demand_pricing.py" (
  echo ERROR: The Phase 5.7.6 pricing regression test is missing.
  goto :failed
)

if not exist "tests\test_no_scanner_watchlist_writes.py" (
  echo ERROR: The Phase 5.7.6 watchlist-safety regression test is missing.
  goto :failed
)
...
set "BACKUP=backups\phase5.7.6-source-before-%STAMP%"
...
echo Temporary API failures receive three different controlled request paths.
echo Only final failures are warnings; recovered retries are reported separately.
echo Live Radar and Random Range Sniper no longer write to the eBay Watchlist.
echo Discovery, workbook output, links and manual Snipe Queue copying are unchanged.
goto :success
