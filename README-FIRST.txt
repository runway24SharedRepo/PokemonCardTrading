...
- Qualifying new listings are deduplicated and added to the main eBay
  Watchlist.
- Matches remain grouped under the associated per-card virtual list in
  PokéBid.

The delivered target is item price plus delivery. The Worker is the automation
engine, so it continues operating when the BAT window or computer is closed.
