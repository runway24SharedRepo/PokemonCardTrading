# Phase 5.7.3 — Windows SQLite Cleanup Fix

This hotfix keeps the Phase 5.7.1 pricing and runtime cache behaviour
unchanged. It corrects the last open SQLite handle in the Phase 5.7.2 test
suite, which prevented Windows from deleting the isolated temporary database.

## What was fixed

- The direct SQLite connection used to age the stale-cache fixture is now
  committed and explicitly closed in `finally` before cleanup.
- The stale-cache test explicitly closes its resolver and renames every SQLite
  database sidecar before cleanup, proving Windows file locks are released.
...
3. Run `install-phase5.7.3-windows-sqlite-cleanup-fix.bat`.
...
are stored in `logs\phase5.7.3-install-test-YYYYMMDD-HHMMSS.log`.
