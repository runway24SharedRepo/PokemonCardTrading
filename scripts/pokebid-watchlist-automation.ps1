...
function Show-Status {
    ...
    $processed = $status.last_search_result
    $processedName = if ($processed -and $processed.search_name) { [string]$processed.search_name } else { $lastSearch }
    $processedAt = if ($processed -and $processed.completed_at) { [string]$processed.completed_at } else { $lastCompleted }
    $processedChecked = if ($processed -and $null -ne $processed.checked) { $processed.checked } else { $searched }
    $processedMatches = if ($processed -and $null -ne $processed.matches) { $processed.matches } else { $newMatches }
    $processedAdded = if ($processed -and $null -ne $processed.added) { $processed.added } else { 0 }
    $processedFailed = if ($processed -and $null -ne $processed.failed) { $processed.failed } else { 0 }
    $processedMode = if ($processed -and $processed.mode) { [string]$processed.mode } else { 'UNKNOWN' }

    Write-Log ("PROCESSED SEARCH | mode={0} | {1} | completed={2} | checked={3} | newly matched={4} | added={5} | failed={6}" -f $processedMode, $processedName, $processedAt, $processedChecked, $processedMatches, $processedAdded, $processedFailed)
    if ($status.current_scan) {
        Write-Log ("CURRENT SEARCH | {0} | started={1}" -f $status.current_scan.search_name, $status.current_scan.started_at)
    }
    Write-Log ("STATUS={0} | policy={1} | saved searches={2} | all matches={3} | watchlisted={4} | historical failed={5}" -f $state, $policy, $rules, $matches, $watchlisted, $failed)
    if ($policy -ne 'per-search-memory-v3; name-and-number; all prices; UK-located; round-robin; max-10') {
        Write-Log 'The deployed Worker does not report the per-search memory-v3 policy. Deploy the latest Worker update before relying on this BAT.' 'WARNING'
    }
}
